...
namespace Game3
{
    ...
    public class Game1 : Game
    {
        ...
        double current—oordinatsX;
        double current—oordinatsY;

        double previous—oordinatsX;
        double previous—oordinatsY;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //
            if (position.X != previous—oordinatsX && position.Y != previous—oordinatsY)
            {
            ...
            previous—oordinatsX = position.X;
            previous—oordinatsY = position.Y;
        ...
        {      
            try 
            {
                if (current—oordinatsY != soursData[current—oordinatsX])
                { 
                    //
                }
            }
            ...
            {
                soursData.Add(current—oordinatsX, current—oordinatsY);
                xString = xString + String.Format("{0:f}", current—oordinatsX) + "             ";
                yString = yString + String.Format("{0:f}", current—oordinatsY) + "             ";
            }
        }
            ...
            foreach (KeyValuePair<double, double> keyValue in polinom)
            {
                ...
                for (int x = varX; x < varX + 5; x++)
                {
                    for (int y = varY; y < varY + 5; y++)
                    {
                        ...
                        if (index > 0 && index <= inGraphData.Length)
                        {    //‚ÒÚ‡ÌÓ‚ÎÂÌÌˇ ÍÓÎ¸ÓÛ Ô≥‰ Ô≥ÍÒÂÎÂÏ
